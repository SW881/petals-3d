...
const WHEEL_SIZE = 200,
    SQUARE_SIZE = 100
const WHEEL_SIZE_M = 128,
    SQUARE_SIZE_M = 64

// ---------- Component ----------
const ColorHexPicker = ({ value, onChange }) => {
    ...
    const wheelSize = isMobile ? WHEEL_SIZE_M : WHEEL_SIZE
    const squareSize = isMobile ? SQUARE_SIZE_M : SQUARE_SIZE
    const squareLeft = (wheelSize - squareSize) / 2

    return (
        <div className="w-full max-w-xs mx-auto font-sans select-none">
            <div className="relative flex items-center justify-between m-[4px]">
                <input
                    type="text"
                    value={inputHex}
                    ...
                    maxLength={7}
                    className="font-bold text-base outline-none border-none py-[4px] px-[8px] bg-[#D3D3D3]/50 rounded-[4px] w-28 text-white"
                    placeholder="#000000"
                />
                <button
                    type="button"
                    onClick={handleEyedropper}
                    className="active:scale-90 cursor-pointer border border-[#D3D3D3] rounded-full p-[4px]"
                    title="Pick color from screen"
                    tabIndex={0}
                >
                    <DropperIcon color="#FFFFFF" size={20} />
                </button>
            </div>
            ...
            >
                ...
                <canvas
                    ...
                    className="absolute block rounded border border-white"
                    style={{
                        left: squareLeft,
                        top: squareLeft,
                        pointerEvents: 'auto',
